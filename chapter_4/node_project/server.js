...
const app = express();
const movie_data = getmovies(); // our variable assigned to the function



const data = { // object to hold movie data
...
app.set('view engine', 'ejs'); // telling to go to views grab the ejs file
...
app.get('/', (req, res) => {
    ...
    console.log("IM ON THE INDEX!!!");
      res.render("pages/index", data);


});

// was return orignally


app.get('/movie/:movieId', (req, res) => {
    let name = req.params.movieId;

    for (var i = 0; i < movie_data.length; i++){

          if (req.params.movieId === movie_data[i].id){
              res.render('pages/movie', {movie: movie_data[i]});
        }
    }
...
app.get('/movie/:movieId', (req, res) => {
      ...
      console.log(" OUR ID!!!!! "+ ID); // will print id exp: 9233233

        for(i = 0; i < movie_data.length; i++){
                if(ID == movie_data[i].id){
                      res.render("./pages/movie", data[i].id); 
          }
        }
});
